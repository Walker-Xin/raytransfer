...
int main()
{
	double spin, spin2, epsilon_r, epsilon_t, defpar;
	double D, D2;
	...
	for (robs = robs_i; robs < robs_f; robs = robs * rstep)
	{
		...
		{
			// ---------- OLD RK45 METHOD STARTS ---------- //
			xobs = robs * cos(pobs);
			yobs = robs * sin(pobs) * cos(iobs);

			/* ----- Ray tracing ----- */

			stop_integration = raytrace(iobs, spin, defpar, dobs, xobs, yobs, xin, xout, traced);
